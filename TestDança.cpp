...
void gen() {
	n = xorRNG() % ( (int)1e6 + 1 ), e = xorRNG() % 1001;
	validAns = false;

	test.clear(); dist.clear(); compI.clear();
...
int mainT() {
	...
	for(it = test.begin(); it != test.end(); it++) {
		...
		fprintf(stderr,"ANSWER FORMULATING & PRINTING TIME FOR CASE IT = %d: %f\n", *it, ((float) tAns) / CLOCKS_PER_SEC);
	}
	...
	fprintf(stderr,"CASE %d:\n %d ticks spent\n <=> %f seconds spent\n\n", caseNum, t, ((float)t) / CLOCKS_PER_SEC);
